...
from gym_minecraft_pygame.minecraft_env import MinecraftConfiguration, ActionSpaceType
from pythomata.dfa import DFA

from minecraft.env import make_env_from_dfa, make_env
...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=str, default="experiments/minecraft-output", help="Output directory for the experiment results.")
    parser.add_argument("--overwrite", action="store_true", default=False, help="Overwrite the content of the output directory.")
    parser.add_argument("--expert-config", type=str, default="minecraft/expert_config.yaml", help="RL configuration for the expert.")
    parser.add_argument("--learner-config", type=str, default="minecraft/learner_config.yaml", help="RL configuration for the learner.")
...
def run_expert(arguments, configuration):
    ...
    config = MinecraftConfiguration(horizon=100, nb_goals=1, action_space_type=ActionSpaceType.TELEPORT)
...
def run_learner(arguments, configuration, dfa: pythomata.dfa.DFA):
    ...
    config = MinecraftConfiguration(horizon=100, nb_goals=1, action_space_type=ActionSpaceType.DIFFERENTIAL)
